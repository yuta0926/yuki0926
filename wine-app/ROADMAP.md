...
- 複数画像対応
- AI確認ステータスの運用フロー整備
- `crud.py` の整理(現状未使用のプレースホルダー)

## 次にやること(短期)

特に緊急のTo Doはなし。中期項目からの着手を検討。
...
3. **`crud.py` の整理**
   ...
   - 実害はないが、方針を「`routers/*.py`に集約する」に倒すか、`crud.py`へ実際に分離するか整理する
...
6. **RLS(Row Level Security)のSupabaseダッシュボード目視確認**
   - `docs/SPEC.md` の「4. データモデル」節にある要確認事項。コード側の情報だけでは判定不可なため、Supabaseダッシュボードでの確認が必要
